...
\]
则可把主要计算转化为矩阵乘法，内存从三维差分降为二维 Gram 矩阵。这一推导解释了为什么实际大规模近邻检索常使用矩阵乘、分块和近似索引。实验一的结论是：Python 适合表达实验逻辑，NumPy 适合承担批量数值内核；两者边界清晰，程序才既可读又高效。

\subsection*{复杂度、数值稳定性与可复现实验习惯}
向量化实验还需要区分“算术复杂度”和“执行复杂度”。双层 Python 循环与 NumPy 广播都执行 $nmd$ 次量级的减法、乘法和加法，但循环版本每次标量运算都包含解释器取指、对象拆箱、类型分派和边界检查；数组版本把这些操作摊入少数连续内存核函数中。若记 Python 单次标量开销为 $c_p$，底层向量化单次算术开销为 $c_v$，则两者可粗略写成
\[
...
\begin{center}
\captionof{table}{Python/NumPy 实验对象与作用边界。}
...
\]
刻画局部变化。虽然该算子比 Sobel 简单，但足以说明视觉模型为何需要局部差分特征：物体边界和纹理常体现在相邻像素的强度变化中。实验一因此把 Python 语法、数组内存和图像预处理连接为后续卷积实验的输入基础。
...
该思想也对应后续深度学习中的 mini-batch。无论是距离矩阵还是神经网络训练，核心都在于把整体经验风险拆成可放入内存的小块，再用批量代数保持硬件效率。Python 层负责编排块循环，NumPy 或 PyTorch 层负责块内密集计算。这个边界划分是本报告所有实验代码的基本设计原则。
...
DataLoader 的作用不仅是按批读取样本，还隐含了随机打乱、批量堆叠和 CPU 到 GPU 的传输边界。对于小图像任务，batch size 过小会导致梯度噪声大，过大则降低更新频率并增加显存压力。实验中的 batch size 64 和 128 展示了这一折中：较大批量在单步估计上更稳定，但短训练内不一定更快达到高准确率。若进一步扩展，可加入学习率 warmup 和余弦退火，使早期稳定性和后期收敛精度同时改善。
...
CIFAR-10 是 $32\times32$ RGB 十类图像分类任务，主体小、背景复杂、类间边界重叠明显。卷积层利用局部连接与参数共享：
\[
...
逐类准确率显示较强类别集中在 @@E3_STRONG@@，薄弱类别集中在 @@E3_WEAK@@。动物类之间常共享毛发纹理和背景，交通工具类则容易受视角、遮挡和背景共现影响。混淆矩阵比总体准确率更有诊断价值：对角线表示稳定识别，非对角高值指出决策边界重叠区域。错误样本网格选取高置信错误样本，显示模型并非只在低质量图像上犯错，也会在背景相关性强或主体过小时做出自信错误。梯度响应图进一步展示模型关注区域：若响应集中在背景而非主体，说明特征学习存在偏置。
...
这些分析为实验五的鲁棒识别埋下基础。标准增强主要覆盖自然变换，如平移、翻转和颜色扰动；对抗扰动则沿损失梯度构造，往往难以被普通增强覆盖。若一个模型在干净 CIFAR-10 上表现较好，却在 FGSM 下明显下降，说明其决策边界贴近训练样本。鲁棒训练正是把这种局部邻域纳入优化目标，用更严格的约束换取稳定性。
...
迁移学习模型的比较还应考虑参数更新比例。只训练分类头时，反向传播只更新最后线性层，训练快且过拟合风险低；全量微调需要更小学习率，因为预训练卷积核包含通用视觉先验，大步长可能破坏已有表示。若硬件和时间允许，可采用分层学习率：分类头学习率较大，高层残差块较小，低层卷积冻结。该策略在保持通用边缘特征的同时，让高层语义适应 CIFAR-10 的类别边界。
...
\]
其中 $K$ 为类别数。它降低模型对单一类别的极端置信度，常能改善校准和泛化。对 CIFAR-10 这类小图像任务，标签噪声、遮挡和主体过小都会导致边界样本存在多义性，标签平滑能缓解过度拟合硬标签的问题。实验五的 ECE 分析与这里的校准思想相呼应，说明分类性能和置信可靠性应同时报告。
...
\]
其中 $c_i$ 是类别，$B_i$ 是边界框，$s_i$ 是置信度，$M_i$ 是实例掩码。该任务同时考察分类、定位、排序和像素级分割，是实验四的重点。
...
检测模型首先在 backbone/FPN 特征图上放置 anchors。RPN 对每个 anchor 预测前景分数与位置偏移，边界框偏移可写为
\[
\begin{aligned}
...
\end{aligned}
\]
这种参数化把不同尺度的框回归转化为相对 anchor 的标准化回归。RoIAlign 随后对候选区域做双线性采样，避免 RoIPool 中量化坐标带来的错位，使小目标边界更稳定。最终检测头对每个对齐后的 RoI 预测类别分数和边界框偏移，因此 proposal 质量会直接影响最终定位。
...
检测错误可以拆为四类。第一是候选框遗漏，RPN 没有给真实目标产生足够接近的 proposal；第二是分类错误，proposal 定位到目标但类别分数低；第三是框回归误差，类别正确但边界过大或过小；第四是 NMS 或阈值误差，重复框未删或相邻目标被误删。PennFudan 叠图中预测框与真值人体区域大体重合，说明预训练骨干加检测头微调能够迁移到该数据集；若框边界偏离，通常来自遮挡、人体姿态变化或 mask 边界不清。
...
误差 & 类别混淆 & 漏检、误检、重复框、边界偏移。\\
\bottomrule
\end{tabular}
\end{center}

本实验的检测结果表明，预训练检测器在行人实例上具有较强迁移能力，少量样本即可得到较高 IoU 和 recall。与此同时，样本量小意味着指标方差较大，因此报告同时使用指标面板与叠图来解释结果。若进一步追求稳定结论，应在更多图像上重复评估，按目标尺度、遮挡程度和实例数量分组，观察小目标、多人重叠和边界不清场景下的误差变化。
...
\subsection*{FGSM 对抗样本：决策边界的局部线性}
...
\]
MNIST LeNet 在 $\epsilon=0$ 时准确率为 @@FGSM0@@，最大扰动处下降到 @@FGSM3@@。视觉上数字仍可辨认，但高维决策边界已被扰动穿越，说明标准训练并不自动带来局部鲁棒性。
...
\subsection*{额外实验设计}
前四个实验说明标准分类器在干净测试集上可取得较好准确率，但 FGSM 表明模型边界可能贴近样本流形。额外实验在 CIFAR-10 上比较 baseline、强增强和 FGSM 对抗训练，评估 clean accuracy、robust accuracy、ECE 校准误差与推理延迟。目标是从“准确率最高”扩展到“预测稳定、置信可靠、代价可控”的综合评价。
...
鲁棒训练常见现象是 clean accuracy 与 robust accuracy 存在张力。原因在于标准训练只要求样本点 $x$ 分类正确，而鲁棒训练要求 $x$ 周围 $\ell_\infty$ 邻域内的大量点都保持正确，等价于扩大决策边界间隔。该约束会牺牲一部分贴近边界但在干净集上可被正确分类的模式，却提升局部稳定性。实验中 \texttt{fgsm\_train} 在 FGSM 指标上更强，说明内层扰动确实改变了模型对梯度方向的敏感性。
...
\]
FGSM training 则把变换族换成由模型梯度生成的最坏方向。三者的差异在于邻域定义不同：自然增强覆盖语义保持的图像变换，对抗训练覆盖损失上升最快的数值方向。本次结果中 \texttt{strong\_aug} 未显著优于 baseline，说明自然增强不能替代对抗邻域；若对抗训练提高 robust accuracy 同时降低 clean accuracy，则体现局部边界平滑与干净点拟合之间的取舍。
...
\]
这比准确率更接近部署需求。例如一个 90\% 准确率但总以 99\% 置信度输出的模型，在人工复核系统中会错误压低复核优先级。若校准较好，可以设置置信阈值：低于阈值的样本进入人工复核，高于阈值的样本自动通过。鲁棒训练若同时改善 ECE，说明它不仅改变了分类边界，也可能降低过度自信。
...
额外实验还可以加入温度缩放改善校准。给定 logits $z$，温度缩放使用 $p=\operatorname{softmax}(z/T)$，在验证集上选择 $T$ 最小化 NLL。$T>1$ 会软化概率分布，通常降低过度自信。温度缩放不改变预测类别，因此 clean accuracy 不变，但 ECE 可能下降。若与对抗训练结合，可以区分“边界鲁棒性”和“概率校准”两类改进来源。该分析体现了额外实验的深度：鲁棒性、校准和解释性是可分解的评价维度，需要分别设计指标。
...
从误差分析看，不同实验的错误来源具有层次性。NumPy 实验的错误常来自 dtype、shape、广播轴和视图/复制；PyTorch 实验的错误常来自学习率、batch size、训练/评估状态和数据归一化；CIFAR-10 分类的错误常来自类间相似、主体过小、背景偏置和训练预算；检测实验的错误可分解为候选框遗漏、NMS 阈值、框回归偏移和 mask 边界不准；鲁棒实验的错误则与局部梯度、过度自信和扰动半径有关。
...
\section*{综合结论}
本报告完成了从 Python/NumPy 基础、PyTorch 自动微分、CIFAR-10 图像分类，到迁移学习、检测、对抗攻击、生成建模和鲁棒识别的完整实验链路。实验一说明矩阵化表达和内存布局决定数值程序效率；实验二给出深度学习训练闭环和梯度推导；实验三展示卷积归纳偏置、增强和迁移学习对视觉分类的影响；实验四把识别扩展到结构化输出、安全边界和分布生成；实验五进一步从鲁棒性和校准角度审查模型可靠性。由此可见，模式识别系统的质量不能只由单一准确率衡量，还必须同时报告数据流、优化过程、误差结构、鲁棒性、可解释证据和复现条件。

\section*{符号与指标速览}
报告中反复出现的符号可归纳为四组：$x,y$ 表示样本与标签，$\theta$ 表示模型参数，$\ell$ 表示损失函数，$\delta$ 表示输入扰动。分类实验关注 $p(y|x)$ 与交叉熵，检测实验关注边界框 $B$ 与置信排序，生成实验关注潜变量 $z$ 到图像 $G(z)$ 的映射，鲁棒实验关注 $B_\epsilon(x)$ 邻域内的最坏损失。统一符号有助于把多个实验的公式放在同一框架下理解。
